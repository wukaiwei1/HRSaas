...
export default {
  ...
  data() {
    // 判断添加的部门是否和同级部门重复
    const isPartName = async (rule, value, callback) => {
      let isRepeat
      if (this.formData.id) {
        ...
        isRepeat = isRepeat.some((item) => item.name === value)
      } else {
        isRepeat = this.DepartmentData?.children?.some(
  ...
  methods: {
    ...
    async sibmit() {
      ...
      try {
        if (this.formData.id) {
          await changeDeptByIdApi(this.formData)
